...
int main()
{
  ...
  Vector const& y = simulation->fracPowerPerpUpPol.row(dipoleIndex - 1).head(u.size());
  Vector const& yParapPol = simulation->fracPowerParaUpPol.row(dipoleIndex - 1).head(u.size());
