...
void Data::ImportManager::autoSetFileFormat()
{
  ...
  if (!_fin) { throw std::runtime_error("Failed to open file!"); }

  char c;
  while (_fin.get(c)) {
    if (std::isalnum(static_cast<unsigned char>(c))) {
      _ftype = FileFormat::CSV;
      return;
    }
    else if (c == '{') {
      _ftype = FileFormat::JSON;
      return;
    }
  }

  if (_fin.eof()) { throw std::runtime_error("Failed to auto-detect file format!"); }
...
void Data::JSONimporter::setSolverMode() { _mode = _reader.get_mode(); }

Data::SolverMode Data::JSONimporter::getSolverMode() { return (_mode); }

std::unique_ptr<BaseSolver> Data::JSONimporter::solverFromFile() { return _reader.makeSolver(); }