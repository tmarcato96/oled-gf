...
{
  constexpr std::array<char, 4> validDelimiters = {'\t', ' ', ';', ','};
  ...
  while (std::getline(iFile, line)) {
    ...
    if (!(startOfDataFound)) {
      if (!(line.empty()) && std::isdigit(line[0])) {
        // Check delimiters and overwrite if it doesn't match
        auto delimiterInferred =
          std::find_first_of(line.begin(), line.end(), validDelimiters.begin(), validDelimiters.end());
        if (delimiterInferred == line.end()) { throw std::runtime_error("Fitting file has invalid delimiter!"); }
        else if (*delimiterInferred != delimiter) {
          delimiter = *delimiterInferred;
        }
    ...
    while (std::getline(iss, token, delimiter)) {
      ...
      // if (mMode == 0) {mSimulationData.insert(std::pair{data[MAX_COLS - 3], std::complex<double>{data[MAX_COLS - 2],
      // data[MAX_COLS - 1]}});} else {mFittingData.insert(std::pair{data[MAX_COLS - 3], data[MAX_COLS - 1]});}
    }
    cols.push_back(col);
  }
