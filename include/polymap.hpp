...
template<typename Values> struct PolyMap
{
  using value_type = typename Values::map_types;
  ...
  template<typename... Keys> auto operator()(Keys&&... keys) const
  {
    return KeySelectionProxy<Values>(*this, {std::forward<Keys>(keys)...});
  }
...
template<typename Values> struct KeySelectionProxy
{
  ...
  // basic operations
  template<typename T> KeySelectionProxy& operator+(const T& increment) const
  {
    auto newOps = operations;
  ...
  template<typename T> KeySelectionProxy& operator+=(const T& increment)
  {
    operations.emplace_back(makeSumOp<T>(increment));
    evaluate();
    operations.clear();
  ...
  template<typename T> KeySelectionProxy& operator-(const T& decrement) const
  {
    auto newOps = operations;
  ...
  template<typename T> KeySelectionProxy& operator-=(const T& decrement)
  {
    operations.emplace_back(makeSubOp<T>(decrement));
    evaluate();
    operations.clear();
  ...
  template<typename T> KeySelectionProxy& operator*(const T& factor) const
  {
    auto newOps = operations;
  ...
  template<typename T> KeySelectionProxy& operator*=(const T& factor)
  {
    operations.emplace_back(makeMultOp<T>(factor));
    evaluate();
    operations.clear();
  ...
  template<typename T> KeySelectionProxy& operator/(const T& denominator) const
  {
    auto newOps = operations;
  ...
  template<typename T> KeySelectionProxy& operator/=(const T& denominator)
  {
    operations.emplace_back(makeDivOp<T>(denominator));
    evaluate();
    operations.clear();
...
private:
  // operation function generators
  template<typename T> auto makeSumOp(const T& operand)
  { // nested lambda here avoids headaches. Other approaches weren't as effective
  ...
  template<typename T> auto makeSubOp(const T& operand)
  {
    return [=](typename Values::map_types& val) {
      std::visit(
        [&](auto& v) {
  ...
  template<typename T> auto makeMultOp(const T& operand)
  {
    return [=](typename Values::map_types& val) {
      std::visit(
        [&](auto& v) {
  ...
  template<typename T> auto makeDivOp(const T& operand)
  {
    return [=](typename Values::map_types& val) {
      std::visit(
        [&](auto& v) {
...
// out of class operator definitions to allow for lhs operations
template<typename T, typename Values> auto operator+(const T& lhs, const KeySelectionProxy<Values>& rhs)
{
  return rhs + lhs; // Uses the existing member operator
}

template<typename T, typename Values> auto operator+=(T& lhs, const KeySelectionProxy<Values>& rhs)
{
  return rhs += lhs;
}
